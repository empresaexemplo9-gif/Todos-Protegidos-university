...
const money = (v, allowNeg = false) => {
  const n = typeof v === "number" && Number.isFinite(v) ? v : 0;
  return Math.round((allowNeg ? n : Math.max(0, n)) * 100) / 100;
};
...
function pickArray(payload, listPath) {
...
function collectFields(obj, prefix = "", out = [], depth = 0) {
...
function normalizeAvaConfig(input, prev = {}) {
  input = input || {};
  const prevByName = {};
  for (const h of prev.headers || []) prevByName[h.name] = h.value;
  const headers = (Array.isArray(input.headers) ? input.headers : []).map((h) => {
  ...
  return {
    ...
    map: {
      name: clean(map.name, 160), sku: clean(map.sku, 160), brand: clean(map.brand, 160), model: clean(map.model, 160),
      category: clean(map.category, 160), unit: clean(map.unit, 160), purchasePrice: clean(map.purchasePrice, 160),
      salePrice: clean(map.salePrice, 160), description: clean(map.description, 160),
    },
  };
}
...
async function avaFetchPage(config, pageValue) {
  ...
  try { return JSON.parse(text); } catch { throw new Error("A resposta do AVA não veio em JSON. Confira a URL e a autenticação."); }
}
async function avaFetchRecords(config, { sample = false } = {}) {
...
function mapAvaRecord(record, map, actor, now) {
