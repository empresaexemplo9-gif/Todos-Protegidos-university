...
// Migrações idempotentes aplicadas sempre (inclusive em bancos já criados),
// pois o schema acima só roda na primeira criação das tabelas.
const MIGRATIONS = [
  `ALTER TABLE sona_catalog_items ADD COLUMN IF NOT EXISTS image_url text NOT NULL DEFAULT ''`,
];
...
export function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      const [row] = await sql`SELECT to_regclass('public.sona_proposals') AS tabela`;
      if (!row?.tabela) { for (const statement of SCHEMA) await execute(statement); }
      for (const statement of MIGRATIONS) await execute(statement);
    })().catch((error) => {
