...
import { catalogImages, catalogImagesVersion } from "../server/catalog-images.mjs";
import {
...
export const mapCatalogItem = (i) => ({
  ...
  purchasePrice: i.purchase_price, salePrice: i.sale_price, imageUrl: i.image_url || "",
...
/* ------------------------------ catálogo ------------------------------ */
const CATALOG_SEED_VERSION = `2026-08-21-imagens-reais-${catalogImagesVersion}-${CATALOG_CLASSIFICATION_VERSION}`;
...
export async function seedCatalog() {
  ...
  for (const sourceItem of initialCatalog) {
    ...
    const imageUrl = item.imageUrl || catalogImages[item.id] || "";
    await sql`
      INSERT INTO sona_catalog_items (id, kind, name, category, brand, model, sku, system,
        description, source_url, unit, purchase_price, sale_price, image_url, updated_by, created_at, updated_at)
      VALUES (${item.id}, ${item.kind}, ${item.name}, ${item.category || ""}, ${item.brand || ""},
              ...
              ${item.purchasePrice || 0}, ${item.salePrice || 0}, ${imageUrl},
              ${item.updatedBy || "Equipe Sona"}, ${now}, ${now})
      ON CONFLICT (id) DO UPDATE SET
        kind = EXCLUDED.kind,
        category = EXCLUDED.category,
        image_url = CASE
          WHEN sona_catalog_items.image_url = '' AND EXCLUDED.image_url <> '' THEN EXCLUDED.image_url
          ELSE sona_catalog_items.image_url
        END,
        updated_at = CASE
          WHEN sona_catalog_items.kind IS DISTINCT FROM EXCLUDED.kind
            OR sona_catalog_items.category IS DISTINCT FROM EXCLUDED.category
            OR (sona_catalog_items.image_url = '' AND EXCLUDED.image_url <> '') THEN EXCLUDED.updated_at
          ELSE sona_catalog_items.updated_at
        END`;
  }

  // Também corrige itens importados, linhas de orçamentos e propostas já salvas.
  // Assim a nova taxonomia vale imediatamente para os dados existentes.
  const catalogRows = await sql`SELECT * FROM sona_catalog_items WHERE kind <> 'plan-image'`;
  for (const current of catalogRows) {
    const item = normalizeCatalogItem(mapCatalogItem(current));
    if (current.kind !== item.kind || current.category !== item.category) {
      await sql`UPDATE sona_catalog_items SET kind = ${item.kind}, category = ${item.category} WHERE id = ${current.id}`;
    }
  }
  const budgetRows = await sql`SELECT * FROM sona_budget_items`;
  for (const current of budgetRows) {
    const item = normalizeCatalogItem(mapLine(current));
    if (current.kind !== item.kind || current.category !== item.category) {
      await sql`UPDATE sona_budget_items SET kind = ${item.kind}, category = ${item.category} WHERE id = ${current.id}`;
    }
  }
  const proposalRows = await sql`SELECT id, data FROM sona_proposals`;
  for (const current of proposalRows) {
    const data = normalizeProposalData(current.data);
    if (JSON.stringify(data) !== JSON.stringify(current.data)) {
      await sql`UPDATE sona_proposals SET data = ${JSON.stringify(data)}::jsonb WHERE id = ${current.id}`;
    }
  }
...
export async function listCatalogItems() {
  const rows = await sql`SELECT * FROM sona_catalog_items`;
  return rows.map(mapCatalogItem).sort(compareCatalogItems);
}
...
export async function clearCatalogItems() {
  await sql`DELETE FROM sona_catalog_items`;
}
...
export async function insertBudgetItem(line) {
  line = normalizeCatalogItem(line);
  await sql`
...
export async function saveProposalDraft(existing, values, actor) {
  ...
  const proposalData = normalizeProposalData(values.data);
  if (existing) {
    const rows = await sql`
      UPDATE sona_proposals SET code = ${values.code}, client = ${values.client},
        project = ${values.project}, data = ${JSON.stringify(proposalData)}::jsonb,
  ...
  const rows = await sql`
    ...
    VALUES (${randomUUID()}, ${values.code}, ${values.client}, ${values.project}, 'draft',
            ${JSON.stringify(proposalData)}::jsonb, ${values.manualHtml}, NULL, '', '', '', NULL,
            ${actor}, ${actor}, ${now}, ${now})
    RETURNING *`;
  return rows[0];
}
